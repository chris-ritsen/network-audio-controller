...
class ChannelListCommand(Command):
    ...
    def filter_devices(self, devices):
        if self.option('device-name'):
            devices = dict(filter(lambda d: d[1].name == self.option('device-name'), devices.items()))
            ...
            if possible_names.intersection(set(devices.keys())):
                devices = dict(filter(lambda d: d[1].server_name in possible_names, devices.items()))
            else:
                try:
