...
class SubscriptionAddCommand(Command):
    ...
    async def subscription_add(self):
        ...
        if self.option('tx-device-name'):
            tx_device = next(filter(lambda d: d[1].name == self.option('tx-device-name'), dante_devices.items()))[1]
        ...
        if self.option('rx-device-name'):
            rx_device = next(filter(lambda d: d[1].name == self.option('rx-device-name'), dante_devices.items()))[1]
        elif self.option('rx-device-host'):
            rx_device = next(filter(lambda d: d[1].ipv4 == self.option('rx-device-host'), dante_devices.items()))[1]
        ...
        if rx_channel and rx_device and tx_channel and tx_channel:
            self.line(f'{rx_channel.name}@{rx_device.name} <- {tx_channel.name}@{tx_device.name}')
