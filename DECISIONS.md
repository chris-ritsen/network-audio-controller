...
- **ABI discipline:** any FFI signature change bumps `NETAUDIO_ABI_VERSION` in `ffi.rs` and `binding.py` together, plus `make header`. The binding refuses to load a mismatched library rather than segfault.
