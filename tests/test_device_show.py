...
def test_format_aes67_includes_prefix_without_changing_enable_state():
    ...
    assert device_display._format_aes67(device) == "off (multicast prefix 239.238.0.0)"


def test_format_channel_count_does_not_turn_unknown_into_zero():
    assert device_display._format_channel_count({}, None) == "unknown"
    assert device_display._format_channel_count({}, 0) == "0"
    assert device_display._format_channel_count({1: object()}, 64) == "1"


@pytest.mark.parametrize(
    ("clock_frequency_offset_parts_per_billion", "expected"),
    [(-25_473, "-25.473 ppm"), (-21_200, "-21.2 ppm"), (-178, "-0.178 ppm"), (0, "0 ppm"), (1_601, "1.601 ppm")],
)
