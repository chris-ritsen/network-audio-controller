...
export const deviceFixture = JSON.parse(
...
export async function serveWebapp(
  page,
  {
    ...
    issues = { schema_version: 1, count: 0, active_count: 0, issues: [] },
    onClearEventJournal = () => {},
  } = {},
) {
  await page.route("**/*", async (route) => {
    ...
    if (url.pathname === "/events" && request.resourceType() !== "document") {
      return route.fulfill({
        ...
        body: `data: ${JSON.stringify({ event: "snapshot", devices, metering })}\n\n`,
      });
    }
    if (url.pathname === "/event-journal") {
      if (request.method() === "DELETE") {
        ...
        return route.fulfill({
          ...
          body: JSON.stringify({
            cleared: eventJournal.events.length,
      ...
      return route.fulfill({
        contentType: "application/json",
        body: JSON.stringify(eventJournal),
      });
    }
    ...
    if (request.method() !== "GET")
      return route.fulfill({
        status: 503,
        contentType: "application/json",
        body: '{"error":"No test handler for this action"}',
      });
    const path = url.pathname.split("/").at(-1).includes(".")
