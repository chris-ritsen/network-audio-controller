...
test("product model and Dante platform name remain distinct", () => {
  const device = {
    ...
    platform_model_name: "Brooklyn II",
    platform_model_identifier: "Bklyn2",
  };
  ...
  const unavailable = render(
    h(StatusSection, {
      device: { ...device, platform_model_name: undefined },
    }),
  );
...
test("reported Bluetooth product name appears in device list, header and search", () => {
  const device = {
    ...
    name: "avio-bt-1",
    server_name: "avio-bt-1.local.",
    model: "",
    dante_model: "Audinate Dante AVIO Bluetooth",
    online: true,
  };
  ...
  assert.match(
    format.deviceSummaryLine(device),
    /Audinate Dante AVIO Bluetooth/,
  );
  for (const path of ["/devices", "/devices/avio-bt-1/receive"]) {
    setLocation(path);
    const markup = render(
      h(devicesView.component, { location: router.resolve(path, "") }),
    );
  ...
  assert.equal(
    format.deviceModelName({ model: "AD4D", dante_model: "other" }),
    "AD4D",
  );
