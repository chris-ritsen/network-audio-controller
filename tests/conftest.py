...
def load_fixture():
    def _load_fixture(filename: str) -> bytes:
        ...
        if not fixture_path.exists():
            pytest.fail(f"Fixture file not found: {fixture_path} (looked in {FIXTURES_DIR.resolve()})")
...
def check_generated_command_payload(
    ...
    if not isinstance(generated_hex_payload, bytes):
        pytest.fail(f"For {context_id}, expected bytes payload but got {type(generated_hex_payload).__name__}")
