...
REQUEST_FIXTURE_GET_RECEIVERS = "20250517_200646_499097_avio-usb-2_get_receivers_request.bin"
RESPONSE_FIXTURE_GET_RECEIVERS = "20250517_200646_499097_avio-usb-2_get_receivers_response.bin"
...
def test_parse_get_receivers_response_payload(load_fixture, test_case: GetReceiversTestCase):
    raw_response_data = load_fixture(test_case.response_fixture)
    hex_rx_response = raw_response_data.hex()
    parsed_channels_info = []

    for i in range(0, min(test_case.num_to_parse, MAX_CHANNELS_PER_RECEIVER_PAGE_PARSE)):
        block_start_hex = RECEIVERS_RESPONSE_HEADER_SIZE_HEXCHARS + (i * RECEIVERS_CHANNEL_BLOCK_SIZE_HEXCHARS)
        ...
        fields = [channel_data_hex[j : j + 4] for j in range(0, len(channel_data_hex), 4)]
        ...
        tx_ch_name = (
            _get_label_for_test(hex_rx_response, tx_ch_name_offset_hex) if tx_ch_name_offset_hex != "0000" else rx_name
        )
