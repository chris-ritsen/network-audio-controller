...
def test_provenance_bundle_resolution_prefers_an_explicit_existing_path(tmp_path, monkeypatch):
    ...
    assert capture_options._resolve_provenance_bundle_path(str(explicit_bundle)) == explicit_bundle
    assert capture_options._resolve_provenance_bundle_path("missing.tar.gz") == (
