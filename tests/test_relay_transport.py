...
def bonjour_relay(monkeypatch):
    ...
    monkeypatch.setattr(relay_module.socket, "gethostname", lambda: "netaudio-test-host")
...
class TestBonjourReconcile:
    @pytest.mark.asyncio
    async def test_long_hostname_produces_bounded_stable_service_identity(self, bonjour_relay, monkeypatch):
        import netaudio.daemon.relay as relay_module

        hostname = "sat12-bq150-7dbdb9ac-5e81-4902-96fc-5a5171b62dcd-F2CBA1AA41D9"
        monkeypatch.setattr(relay_module.socket, "gethostname", lambda: hostname)
        ...
        service_info = bonjour_relay.service_info
        instance_label = service_info.name.removesuffix(f".{relay_module.RELAY_SERVICE_TYPE}")
        assert instance_label == "netaudio-relay (sat12-bq150-7dbdb9ac-5e81-4902-96-4b24fff9a31b)"
        assert len(instance_label.encode("utf-8")) == relay_module.SERVICE_LABEL_MAXIMUM_BYTES
