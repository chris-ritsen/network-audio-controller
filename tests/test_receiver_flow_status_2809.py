...
def test_query_builder_and_command_factory_are_byte_identical_to_shipping_controller():
    ...
    command, service = DanteDeviceCommands().command_query_receiver_flow_status_2809(0x2856)
...
def test_parser_exposes_flow_format_latency_endpoint_and_receiver_mapping():
    page = core.parse_response("receiver_flow_status_page_2809", _packet(0x3600, 4))
    ...
    assert page["flows"] == [
        {
            ...
            "status_flags_at_record_offset_60": 0x0400,
            "status_code_at_record_offset_62": 0x0101,
            ...
            "raw_record_hexadecimal": page["flows"][0]["raw_record_hexadecimal"],
        }
    ]
    assert len(page["flows"][0]["raw_record_hexadecimal"]) == 168

    two_receiver_page = core.parse_response("receiver_flow_status_page_2809", _packet(0x3600, 10))
    ...
    empty_page = core.parse_response("receiver_flow_status_page_2809", _packet(0x3600, 12))
...
def test_parser_fails_closed_on_structural_corruption_and_a32_rejection():
    ...
    for start, end, replacement in corruptions:
        ...
        with pytest.raises(core.NetaudioCoreError):
            core.parse_response("receiver_flow_status_page_2809", bytes(malformed))
    ...
    with pytest.raises(core.NetaudioCoreError):
        core.parse_response("receiver_flow_status_page_2809", a32_response)
