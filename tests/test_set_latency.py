...
def _extract_tarball(name):
    ...
    archive_name = {
        "session_27_set_latency_full_provenance": "set_latency_full_provenance",
        "session_28_set_latency_full_provenance_step2": "set_latency_full_provenance_step2",
    }.get(name, name)
    archive = FIXTURES_ROOT / f"{archive_name}.tar.gz"
    if not archive.exists():
        pytest.skip(f"fixture archive {archive.name} not present on this machine", allow_module_level=True)
    temporary_directory = Path(tempfile.mkdtemp()).resolve()
    with tarfile.open(archive, "r:gz") as tar_archive:
        for member in tar_archive.getmembers():
            destination = (temporary_directory / member.name).resolve()
            if destination != temporary_directory and temporary_directory not in destination.parents:
                raise RuntimeError(f"fixture archive path escapes extraction directory: {member.name}")
        extraction_arguments = {"filter": "data"} if sys.version_info >= (3, 12) else {}
        tar_archive.extractall(temporary_directory, **extraction_arguments)
    extracted = temporary_directory / name
...
@pytest.mark.skipif(not core.available(), reason="netaudio-core not built")
@pytest.mark.parametrize("target_ns", ALL_VALUES_NS)
def test_core_builder_matches_provenance_capture(target_ns):
    directory = session_dir(target_ns)
    manifest = load_manifest(directory)
    captured_request = load_bin(directory, samples_by(manifest, "0x1101", "request")[0]["file"])

    packet, _ = DanteDeviceCommands().command_set_latency(target_ns / 1_000_000.0)

    assert len(packet) == len(captured_request) == 40
    assert packet[2:4] == captured_request[2:4]
    assert packet[6:] == captured_request[6:]
...
class TestEvidenceChain:
    ...
    def test_session_27_evidence_included_in_session_28_bundle(self):
        manifest = load_manifest(SESSION_28)
        step1_evidence = [s for s in manifest["samples"] if s.get("evidence") and s.get("session_id") == 27]
