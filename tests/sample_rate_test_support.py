...
def make_fake_sample_rate_change(send, probe_sample_rate_status):
    async def change_sample_rate_with_command_sender(
        ...
        current_sample_rate, supported_sample_rates = await probe_sample_rate_status(device.ipv4)
        ...
        packet, _, port = DanteDeviceCommands().command_set_sample_rate(sample_rate)
        try:
        ...
        observed_sample_rate, observed_supported_sample_rates = await probe_sample_rate_status(device.ipv4)
    ...
    return change_sample_rate_with_command_sender
