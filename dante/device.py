...
class DanteDevice():
    ...
    def set_channel_name(self, channel_type, channel_number, new_channel_name):
        response = self.dante_command(*self.command_set_channel_name(channel_type, channel_number, new_channel_name))
        return response


    def identify(self):
        response = self.dante_command(*self.command_identify())
        return response


    def set_latency(self, latency):
        response = self.dante_command(*self.command_set_latency(latency))
        return response


    def set_gain_level(self, channel_number, gain_level, device_type):
        response = self.dante_command(*self.command_set_gain_level(channel_number, gain_level, device_type))
        return response


    def set_encoding(self, encoding):
        response = self.dante_command(*self.command_set_encoding(encoding))
        return response


    def set_sample_rate(self, sample_rate):
        response = self.dante_command(*self.command_set_sample_rate(sample_rate))
        return response


    def add_subscription(self, rx_channel_number, tx_channel_name, tx_device_name):
        response = self.dante_command(*self.command_add_subscription(rx_channel_number, tx_channel_name, tx_device_name))
        return response


    def remove_subscription(self, rx_channel_number):
        response = self.dante_command(*self.command_remove_subscription(rx_channel_number))
        return response


    def reset_channel_name(self, channel_type, channel_number):
        response = self.dante_command(*self.command_reset_channel_name(channel_type, channel_number))
        return response


    def set_device_name(self, name):
        response = self.dante_command(*self.command_set_device_name(name))
        return response


    def reset_device_name(self):
        response = self.dante_command(*self.command_reset_device_name())
        return response
    ...
    def get_device_controls(self):
        ...
        try:
            if not self.name:
                self.name = self.dante_command(*self.command_device_name())[10:-1].decode('ascii')

            # get reported rx/tx channel counts
            if not self.rx_count or not self.tx_count:
                channel_count = self.dante_command(*self.command_channel_count())
    ...
    def get_volume(self, ipv4, mac, port):
        try:
            ...
            volume_start = self.dante_command(*self.command_volume_start(self.name, ipv4, mac, port))
            ...
            if volume_start:
                while True:
                    try:
                        ...
                        if addr[0] == self.ipv4:
                            volume_stop = self.dante_command(*self.command_volume_stop(self.name, ipv4, mac, port))
    ...
    def get_rx_channels(self):
        ...
        try:
            for page in range(0, max(int(self.rx_count / 16), 1)):
                receivers = self.dante_command(*self.command_receivers(page))
                ...
                for index in range(0, min(self.rx_count, 16)):
                    ...
                    if channel:
                        ...
                        rx_channel_name = self.get_label(hex_rx_response, rx_channel_offset)

                        tx_device_name = self.get_label(hex_rx_response, device_offset)

                        if not channel_offset == '0000':
                            tx_channel_name = self.get_label(hex_rx_response, channel_offset)
                        else:
    ...
    def get_tx_channels(self):
        ...
        try:
            for page in range(0, max(1, int(self.tx_count / 16)), 2):
                tx_friendly_names = self.dante_command(*self.command_transmitters(page, friendly_names=True)).hex()
                ...
                for index in range(0, min(self.tx_count, 32)):
                    ...
                    tx_channel_friendly_name = self.get_label(tx_friendly_names, channel_offset)

                    if tx_channel_friendly_name:
                        tx_friendly_channel_names[channel_number] = tx_channel_friendly_name

            for page in range(0, max(1, int(self.tx_count / 16)), 2):
                transmitters = self.dante_command(*self.command_transmitters(page, friendly_names=False)).hex()
    ...
    def command_string(self, command=None, command_str=None, command_args='0000', command_length='00', sequence1='ff', sequence2='ffff'):
    ...
    def command_set_latency(self, latency):
        ...
        return (self.command_string('set_latency', command_length=command_length, command_str=command_str, command_args=command_args), ARC_SERVICE)
    ...
    def command_add_subscription(self, rx_channel_number, tx_channel_name, tx_device_name):
        ...
        return (self.command_string('add_subscription', command_str=command_str, command_args=command_args), ARC_SERVICE)
    ...
    def command_remove_subscription(self, rx_channel):
        ...
        return (self.command_string('remove_subscription', command_str=command_str, command_length=args_length, command_args=command_args), ARC_SERVICE)


    def command_device_info(self):
        return (self.command_string('device_info'), ARC_SERVICE)


    def command_device_name(self):
        return (self.command_string('device_name'), ARC_SERVICE)


    def command_channel_count(self):
        return (self.command_string('channel_count'), ARC_SERVICE)
    ...
    def command_set_device_name(self, name):
        ...
        return (self.command_string('set_device_name', command_length=args_length, command_args=device_name(name)), ARC_SERVICE)


    def command_reset_device_name(self):
        return (self.command_string('reset_device_name'), ARC_SERVICE)
    ...
    def command_reset_channel_name(self, channel_type, channel_number):
        ...
        return (self.command_string('reset_channel_name', command_str=command_str, command_args=command_args, command_length=args_length), ARC_SERVICE)
    ...
    def command_set_channel_name(self, channel_type, channel_number, new_channel_name):
        ...
        return (self.command_string('set_channel_name', command_str=command_str, command_length=args_length, command_args=command_args), ARC_SERVICE)
    ...
    def command_receivers(self, page=0):
        return (self.command_string('rx_channels', command_args=self.channel_pagination(page)), ARC_SERVICE)
    ...
    def command_transmitters(self, page=0, friendly_names=False):
        ...
        return (self.command_string('tx_channels', command_length=command_length, command_str=command_str, command_args=command_args), ARC_SERVICE)
