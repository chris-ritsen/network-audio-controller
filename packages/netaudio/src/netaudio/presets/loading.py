...
from netaudio.presets.parsing import INTERFACE_MODES, STATIC_INTERFACE_FIELDS
...
def _validate_interface(device_name: str, config: dict) -> None:
    ...
    if mode == "static":
        ...
        try:
            for _, field_name in STATIC_INTERFACE_FIELDS:
                IPv4Address(config[field_name])
            IPv4Network(f"{config['ip_address']}/{config['netmask']}", strict=False)
        except ValueError as exception:
            raise ValueError(f"{device_name}: invalid static IPv4 configuration") from exception
...
async def build_preset_plan(matched_devices: list[MatchedPresetDevice]) -> PresetLoadPlan:
    preflight_errors = []
    device_actions = []
...
async def _read_interface_config(application, device, expected: dict):
    ...
    reported = interfaces[0].get("configured") or interfaces[0]
