...
function SavePreset() {
  ...
  const valid =
    selected.length > 0 &&
    selected.every((id) => scopedDevices.value[id]?.online);
  const toggle = (values, value, enabled) =>
    enabled ? [...values, value] : values.filter((item) => item !== value);
    ...
    <form class="flex flex-col gap-4" onSubmit=${save}>
          ...
          ([id, label]) =>
            html`<label key=${id} class="flex items-center gap-2"
              ><input
                ...
                onChange=${(event) => setSections(toggle(sections, id, event.target.checked))}
              />${label}</label
            >`,
        )}
      </fieldset>
      <div>
        <button
          ...
          disabled=${busy || !ready || !valid || !name.trim() || !sections.length}
        >
          <${Icon}
            name="download"
...
function LoadPreset() {
  ...
  const [storeCurrent, setStoreCurrent] = useState(false);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  ...
  async function open(file) {
    ...
    try {
      ...
      const data = await api.previewPreset({
        xml: content,
        devices: Object.keys(scopedDevices.value),
      });
      if (token !== request.current) return;
      setXml(content);
      setPreview(data);
      setChoices(
        data.devices.map((device) => {
          ...
          return {
            name: device.name,
            include: true,
            target: eligible.length === 1 ? eligible[0].id : "",
          };
        }),
      );
    } catch (failure) {
      if (token === request.current) setError(readable(failure.message));
    } finally {
  ...
  async function apply(event) {
    event.preventDefault();
    if (busy || !confirmed || !valid || !inventoryReady.value || !preview)
      return;
    const token = ++request.current;
    setBusy(true);
    setError("");
    ...
    setConfirmed(false);
    try {
      const data = await api.loadPreset({
        xml,
        digest: preview.digest,
        confirmed: true,
        confirm_destructive: destructive,
        store_current_configuration: storeCurrent,
        targets: Object.fromEntries(
          selected.map((choice) => [choice.name, choice.target]),
        ),
        excluded: choices
