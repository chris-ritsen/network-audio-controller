...
export const api = {
  ...
  getDevice: (name, context) =>
    get(
      `/devices/${encodeURIComponent(name)}${context ? `?context=${encodeURIComponent(context)}` : ""}`,
    ),
  ...
  getManagedDomains: (context) =>
    get(
      `/ddm/domains${context ? `?context=${encodeURIComponent(context)}` : ""}`,
    ),
  ...
  planTransmitFlow: (device, specification) =>
    post("/transmit-flows/plan", { device, specification }),
  ...
  setSampleRate: (device, sampleRate, confirmDestructive) =>
    post("/set-sample-rate", {
      device,
      sample_rate: sampleRate,
      confirm_destructive: confirmDestructive === true,
    }),
  setEncoding: (device, encoding) =>
    post("/set-encoding", { device, encoding }),
  ...
  setAes67MulticastPrefix: (device, prefix) =>
    post("/set-aes67-multicast-prefix", { device, prefix }),
  setSampleRatePullup: (device, rawValue) =>
    post("/set-sample-rate-pullup", { device, raw_value: rawValue }),
  setPreferredLeader: (device, preferred) =>
    post("/set-preferred-leader", { device, preferred }),
  setClockSource: (device, clockSource) =>
  ...
  startMetering: (device, clientId) =>
    post("/metering/start", { device, client_id: clientId }),
  stopMetering: (device, clientId) =>
    post("/metering/stop", { device, client_id: clientId }),
