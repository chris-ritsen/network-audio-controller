...
async def _replay_packet(
    ...
    if not packet:
        typer.echo(f"Error: Packet #{packet_id} not found in database.", err=True)
        raise typer.Exit(1)
    ...
    if not destination_ip or not destination_port:
        typer.echo(f"Error: Packet #{packet_id} has no destination address.", err=True)
        raise typer.Exit(1)
...
def live(
    ...
    try:
        ...
        typer.echo(f"Capture: Fatal error: {exception}", err=True)
        traceback.print_exc()
        raise typer.Exit(1)
    finally:
        signal.signal(signal.SIGINT, original_sigint)
