...
def _parse_field_spec(spec: str) -> dict:
    ...
    if not result["name"] or not result["dtype"] or result["offset"] < 0 or result["length"] <= 0:
        typer.echo(f"Invalid --field definition: {spec!r}", err=True)
        raise typer.Exit(1)
    if len(parts) == 5:
...
def _parse_int_option(value: str | None, option_name: str) -> int | None:
    ...
    try:
        ...
        typer.echo(f"Capture: {option_name} must be an integer or hex value (e.g. 0x2010), got: {value}", err=True)
        raise typer.Exit(1)
...
def _parse_time_filter(value: str | None, store: PacketStore, session_id: int) -> int | None:
    ...
    parts = value.split(":")
    if len(parts) < 2:
        typer.echo(f"Capture: time filter must be HH:MM:SS or HH:MM:SS.fff, got: {value}", err=True)
        raise typer.Exit(1)
    ...
    try:
        ...
        typer.echo(f"Capture: invalid time format: {value}", err=True)
        raise typer.Exit(1)
