...
def _validate_fpga_payload(payload):
    ...
    while word_offset < len(payload):
        ...
        if packet_type == 1:
            ...
            data_words = struct.unpack_from(f">{word_count}H", payload, word_offset) if word_count else ()
    ...
    if not idcodes or any(idcode != 0x04004093 for idcode in idcodes):
        raise ValueError(f"Unexpected Spartan-6 IDCODE values: {[f'{idcode:08x}' for idcode in idcodes]}")
