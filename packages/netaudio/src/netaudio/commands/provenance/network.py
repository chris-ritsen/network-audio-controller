...
def provenance_send(
    ...
    if payload_hex is None and packet_id is None:
        typer.echo("Error: Provide --payload-hex or --packet-id.", err=True)
        raise typer.Exit(1)
    ...
    try:
        ...
        if payload_hex is not None:
            ...
            if not source_packet:
                typer.echo(f"Error: Packet #{packet_id} not found.", err=True)
                raise typer.Exit(1)
...
def _responses_by_request_index(samples: list[dict]) -> dict[int, dict]:
    ...
    for sample in samples:
        ...
        if sample.get("direction") == "request":
            ...
            responses_by_request_index[request_index] = sample
    return responses_by_request_index


def _resolve_replay_target(manifest: dict, samples: list[dict], device_ip: str | None) -> str:
    ...
    if not target_ip:
        typer.echo("Cannot determine target device IP. Use --device-ip.", err=True)
        raise typer.Exit(1)
    return target_ip


def _pair_requests_with_responses(
    samples: list[dict], files: dict[str, bytes], responses_by_request_index: dict[int, dict]
) -> list[dict]:
    request_response_pairs = []
    request_index = 0
    for sample in samples:
        if sample.get("evidence", False) or sample.get("direction") != "request":
            continue
        request_index += 1
        payload = files.get(sample["file"])
        if payload is None:
            continue
        original_response_sample = responses_by_request_index.get(request_index)
...
def _print_replay_dry_run(request_response_pairs: list[dict], target_ip: str) -> None:
    for index, pair in enumerate(request_response_pairs, 1):
        sample = pair["sample"]
        opcode_hex = sample.get("opcode_hex", f"0x{sample.get('opcode', 0):04X}")
        print(
            f"  [{index}/{len(request_response_pairs)}] {opcode_hex} -> {target_ip}:{pair['port']}  {len(pair['payload'])}B"
        )
...
def provenance_replay(
    ...
    responses_by_request_index = _responses_by_request_index(samples)

    if not requests:
        typer.echo("No request packets found in bundle.", err=True)
        raise typer.Exit(1)

    original_name = manifest.get("session", {}).get("name", bundle_path.stem)
    replay_name = session_name or f"replay_{original_name}"
    target_ip = _resolve_replay_target(manifest, samples, device_ip)
    ...
    print(f"  Original responses available: {len(responses_by_request_index)}")
    print()

    request_response_pairs = _pair_requests_with_responses(samples, files, responses_by_request_index)

    if dry_run:
        _print_replay_dry_run(request_response_pairs, target_ip)
        return
