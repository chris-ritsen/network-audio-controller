...
from netaudio._common import ansi, run_command
...
from netaudio.commands import meter_tui
...
async def _read_fresh_cache(ordered: list[str], timeout: float, detailed: bool) -> dict[str, dict]:
    ...
    try:
        ...
        if attempted:
            failed_stops = await meter_tui.stop_metering_attempts(
...
async def run_meter(application, devices, options: MeterViewOptions, timeout: float, snapshot: bool) -> None:
    ...
    if snapshot or use_json:
        ...
        for server_name in sorted(
            ...
            levels = meter_tui.format_meter_sample(target, cache.get(server_name))
            if not levels:
                continue
        ...
        if use_json:
            typer.echo(json.dumps(all_json, indent=2))
        else:
    ...
    await meter_tui.run_meter_tui(filtered, options)
