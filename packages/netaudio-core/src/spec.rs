...
fn build_command(spec: CommandSpec, default_host_mac: [u8; 6]) -> Result<Vec<u8>, SpecError> {
    let packet = match spec {
        ...
        CommandSpec::SetChannelName {
            ...
            name,
            transaction_id,
        } => commands::build_set_channel_name(
            parse_channel_type(&channel_type)?,
            channel_number,
            &name,
            transaction_id,
        ...
        CommandSpec::Reboot { host_mac } => {
            commands::build_reboot(parse_mac(&host_mac, default_host_mac)?)?
        }
        ...
        CommandSpec::EnableAes67 { enabled, host_mac } => {
            commands::build_enable_aes67(enabled, parse_mac(&host_mac, default_host_mac)?)?
        }
        CommandSpec::ProbeInterfaceStatus { host_mac } => {
            commands::build_probe_interface_status(parse_mac(&host_mac, default_host_mac)?)?
        }
        CommandSpec::SetInterfaceDhcp { host_mac } => {
            commands::build_set_interface_dhcp(parse_mac(&host_mac, default_host_mac)?)?
        }
