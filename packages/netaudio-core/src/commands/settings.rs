...
pub fn build_reboot(mac: [u8; 6], message_id: u16) -> Result<Vec<u8>, NetaudioError> {
    build_system_reset(mac, message_id, SYSTEM_RESET_MODE_REBOOT)
}

pub fn build_factory_reset(mac: [u8; 6], message_id: u16) -> Result<Vec<u8>, NetaudioError> {
    build_system_reset(mac, message_id, SYSTEM_RESET_MODE_FACTORY)
}
...
fn build_clear_configuration_request(
    ...
    settings_packet(message_id, mac, SETTINGS_SUFFIX_CLEAR_CONFIGURATION, &tail)
}
...
pub fn build_probe_clear_configuration_status(
    ...
    build_clear_configuration_request(mac, message_id, 0)
}
...
pub fn build_clear_all_configuration(
    ...
    build_clear_configuration_request(mac, message_id, CLEAR_CONFIGURATION_ACTION_ALL)
}
...
pub fn build_identify(message_id: u16) -> Result<Vec<u8>, NetaudioError> {
    if message_id == 0 {
        return Err(NetaudioError::InvalidSequence);
    }
    settings_packet(
...
pub fn build_set_encoding(encoding: u32, message_id: u16) -> Result<Vec<u8>, NetaudioError> {
    if encoding == 0 {
...
pub fn build_probe_sample_rate(
    host_mac: [u8; 6],
    message_id: u16,
) -> Result<Vec<u8>, NetaudioError> {
...
pub fn build_probe_sample_rate_pullup(
    ...
    build_sample_rate_pullup_control(host_mac, message_id, 0, 0)
}
...
pub fn build_probe_aes67(mac: [u8; 6], message_id: u16) -> Result<Vec<u8>, NetaudioError> {
    let mut tail = vec![0x10, 0x06, 0x00, 0x00, 0x00, 0x64];
...
fn build_conmon_export_request(
    ...
    settings_packet(message_id, mac, SETTINGS_SUFFIX_DIAGNOSTIC_EXPORT, &tail)
}
