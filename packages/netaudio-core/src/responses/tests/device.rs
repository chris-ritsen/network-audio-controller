...
fn device_settings_preserves_full_variable_width_referenced_values() {
    let control = decode_hexadecimal(
    ...
    let treatment = decode_hexadecimal(
...
fn result_code_reads_header_field() {
    ...
    assert_eq!(
        parse_result_code(&decode_hexadecimal("2809000a284a34000030")),
        Some(0x0030)
    );
    assert_eq!(
        parse_result_code(&decode_hexadecimal("2809000a284c34010030")),
        Some(0x0030)
    );
    assert_eq!(
        parse_result_code(&decode_hexadecimal("2809000a022526000030")),
        Some(0x0030)
    );
